...
def test_diff() -> None:
    ...
    *(0 added, 2 removed, 1 updated, 4 not changed)*<small>Generated by diff-poetry-lock {__version__}</small>
    """
    assert (
        format_comment(
            summary,
...
def test_diff_2() -> None:
    ...
    *(2 added, 0 removed, 1 updated, 4 not changed)*<small>Generated by diff-poetry-lock {__version__}</small>
    """
    assert (
        format_comment(
            summary,
...
def test_file_loading_missing_file_base_ref(cfg: Settings) -> None:
    with requests_mock.Mocker() as m:
        m.get(
            ...
            request_headers={"Authorization": f"token {cfg.token}", "Accept": "application/vnd.github+json"},
            status_code=404,
        )
...
def mock_list_comments(m: Mocker, s: Settings, response_json: list[dict[Any, Any]]) -> None:
    m.get(
        ...
        request_headers={"Authorization": f"token {s.token}", "Accept": "application/vnd.github+json"},
        json=response_json,
    )
